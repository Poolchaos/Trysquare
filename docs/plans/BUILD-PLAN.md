...
| WP-B | Data layer: full 02 schema, migrations, repositories, state-machine enforcement + tests                                                                                                                                                                                                | WP-A       | TODO   |
| WP-C | Git layer: URL validation, bare clone, fetch, live branch listing, merge-base, worktree lifecycle (single and linked dual-worktree layout), bundle builder incl. links.json and changed-exported-symbols extraction, diff/hunk inventory parser + exhaustive fixtures                  | WP-A       | TODO   |
| WP-D | Ruleset domain: rule/directive model, composition function with snapshot tests, protocol importer + fidelity gate (permanent test), markdown export round-trip                                                                                                                         | WP-B       | TODO   |
| WP-E | Engine: EngineAdapter, headless claude runner (CLAUDECODE-scrubbed env, explicit --system-prompt and --tools, full model ids), stream-json parsing, error classes, timeout guard, limit pause, model registry + cheap probing + auth status check per 06; fake claude binary for tests | WP-A       | TODO   |
